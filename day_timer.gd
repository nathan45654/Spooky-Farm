...
signal set_clock_text(clock_string: String)
var game_hours
var game_minutes
var wake_up_time
...
func get_game_time():
	...
	print_to_clock(game_hours, game_minutes)	

func start_day():
	start(seconds_in_a_day)
	
func print_to_clock(hours, minutes):
	var clock_string = ""
	if game_minutes < 10:
		clock_string = str(game_hours) + ":0" + str(game_minutes)
	else:
		clock_string = str(game_hours) + ":" + str(game_minutes)

	set_clock_text.emit(clock_string)
